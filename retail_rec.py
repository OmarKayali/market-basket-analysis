...
def build_customer_item_matrix(df: pd.DataFrame, binarize: bool = True) -> pd.DataFrame:
    """
    Build Customer × Item matrix; if binarize=True then convert to True/False (whether purchased or not)
...
def cluster_customers_dbscan(
    ...
    """
    Input: Customer × Item (boolean) matrix
...
# ---------- Build Basket × Item Matrix ----------
def build_basket_matrix(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Convert transactions into Order × Item quantity matrix and its binary version
...
def rules_for_cluster(
    ...
    """
    Filter customers belonging to a cluster -> Build basket × item for that cluster -> FP-Growth -> Association rules
